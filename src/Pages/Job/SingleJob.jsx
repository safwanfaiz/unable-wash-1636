import { AlertDialog, AlertDialogBody, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogOverlay, Box, Button, Container, Flex, Image, Input, Skeleton, Spinner, Stack, Text, useDisclosure, useToast } from '@chakra-ui/react'
import React from 'react'
import { useEffect } from 'react'
import { useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { NavLink, useNavigate, useParams } from 'react-router-dom'
import { deleteJobsData, getJobsData } from '../../Redux/Job/action'
...
const SingleJob = () => {
  ...
  return (
    <>
       ...
       <Flex w={{lg:"90%"}} margin="auto" gap={"20px"} p={"10px"} mt="70px" mb="50px">
          ...
          <Box boxShadow='base' p='6' rounded='md' bg='white' width={["95%","80%"]} margin="auto">
            <Flex gap={"20px"} padding={"10px"} borderBottom="2px dashed rgb(224, 224, 224)">
               ...
               <Box>
                ...
                <Text fontSize={"0.875rem"} color="rgba(0, 0, 0, 0.54)">Job Opportunity</Text>
                <Text fontSize={"0.875rem"} color={"#3C4858"}><span style={{fontSize:"0.875rem",color:"rgba(0, 0, 0, 0.54)"}} >Job Category: </span> 
                <span style={{color:"rgba(0, 0, 0, 0.54)",fontWeight:"bold"}}    >{current.jobCategory}</span> </Text>
               </Box>
          ...
          <Box mt={"10px"}>
            ...
            <Box mt={"10px"} marginTop="20px">
               <Text fontWeight="bold" color="#3C4858" fontSize={"0.875rem"} mb="20px">Job Description:</Text>
     ...
     <NavLink to={`/jobs/edit/${+id}`}> <Button mr={"5px"} padding="0px 20px">Edit Post</Button>    </NavLink>
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <Button colorScheme='red' onClick={onClose} ml={3}>
               ...
               <NavLink to="/jobs"><Button bg={"none"} _hover={{bg:"#becedc"}} color="#087CDD">BACK TO JOBS</Button></NavLink>
               <Button bg={"#DF1E2E"} color="white" borderRadius="5px" _hover={{bg:"#ab1925"}} onClick="handleApply">APPLY</Button>
            </Flex>
