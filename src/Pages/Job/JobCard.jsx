import { Box, Image, Text,Flex, Button, Input } from '@chakra-ui/react'
  ...
  return (
    <>
       ...
       <Flex padding={"20px"} borderBottom="2px dashed rgb(224, 224, 224)" justifyContent={"space-between"} flexDirection={["column","column","row","row"]}>
          <Flex gap={"20px"}>
               <Box>
                <Image src={job.clogo} w="90px" h="90px"/>
               </Box>

               <Box>
                <Text color={"#3C4858"} fontSize="20px" fontWeight={400}>{job.jobName} </Text>
                <Text fontSize={"0.875rem"} color={"#3C4858"}>{job.cname}</Text>
                <Text fontSize={"0.875rem"} color="rgba(0, 0, 0, 0.54)">Job Opportunity</Text>
                <Text fontSize={"0.875rem"} color="rgba(0, 0, 0, 0.54)">{job.jobCategory}</Text>
                <Text fontSize={"0.875rem"} color="rgba(0, 0, 0, 0.54)">{job.jobLocations}</Text>
                <Text fontSize={"0.875rem"} color={"#3C4858"}><span style={{fontSize:"0.875rem",color:"rgba(0, 0, 0, 0.54)"}}    >Stipend:</span> 
                
                <span style={{color:"rgba(0, 0, 0, 0.54)",fontWeight:"bold"}}    >{job.stipend} </span> </Text>
               </Box>
            ...
            <Box  textAlign={["left","left","right","right"]}>
             ...
             <NavLink to={`/jobs/${job.id}`} > <Text color='#087CDD' fontSize={"0.875rem"} fontWeight="500">VIEW AND APPLY</Text> </NavLink>
          </Box>
       </Flex>
    </>
  )
